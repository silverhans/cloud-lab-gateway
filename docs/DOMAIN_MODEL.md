...
| `name` | string | |
| `ki_domain_id` | string | ID домена в КИ. Не уникален — см. инвариант ниже |
| `created_at` | timestamptz | |

**Инвариант (обновлён по реальности хакатона)**: курс мапится на КИ-домен,
**многие курсы могут мапиться на один и тот же домен**. Это отступление от
исходной формулировки кейса ("1 курс = 1 КИ-домен") обусловлено реальностью
учебного кластера: команде выдаётся один общий домен (например, `Hackhaton`),
внутри которого мы держим логический пул проектов. Логическая изоляция между
курсами сохраняется на application-уровне через RBAC и per-course view scoping.

См. миграцию `0002_relax_course_domain_unique.sql` — снимает UNIQUE-ограничение
на `courses.ki_domain_id`, оставляя обычный индекс для быстрого поиска по
домену.
